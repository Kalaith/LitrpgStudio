...
interface SeriesActions {
  ...
  updateCharacterInSeries: (
    seriesId: string,
    characterId: string,
    updates: Partial<SharedElements["characters"][0]>,
  ) => void;
  ...
  addCharacterRelationship: (
    seriesId: string,
    characterId: string,
    relationship: CrossBookRelationship,
  ) => void;
  ...
  addTimelineEvent: (
    seriesId: string,
    event: Omit<SeriesTimelineEvent, "id">,
  ) => void;
  updateTimelineEvent: (
    seriesId: string,
    eventId: string,
    updates: Partial<SeriesTimelineEvent>,
  ) => void;
  ...
  updateTerm: (
    seriesId: string,
    termId: string,
    updates: Partial<SeriesTerminology>,
  ) => void;
    ...
    (set, get) => ({
      ...
      createSeries: async (seriesData) => {
        ...
        try {
          ...
          if (response.success && response.data) {
            ...
            set({
              error: response.error || "Failed to create series",
              loading: false,
            });
      ...
      updateSeries: async (seriesId, updates) => {
        ...
        try {
          ...
          if (response.success && response.data) {
            ...
            set({
              error: response.error || "Failed to update series",
              loading: false,
            });
          }
        } catch (error) {
            ...
            set((state) => ({
              ...
              currentSeries:
                state.currentSeries?.id === seriesId
                  ...
                  : state.currentSeries,
              analytics: new Map(
                [...state.analytics].filter(([id]) => id !== seriesId),
              ),
              loading: false,
            }));
          } else {
            set({
              error: response.error || "Failed to delete series",
              loading: false,
            });
          }
        } catch (error) {
        ...
        set((state) => ({
          ...
          currentSeries:
            state.currentSeries?.id === seriesId
              ? {
                  ...state.currentSeries,
                  books: [...state.currentSeries.books, newBook].sort(
                    (a, b) => a.bookNumber - b.bookNumber,
                  ),
                  updatedAt: new Date(),
                }
          ...
          series: state.series.map((s) =>
            s.id === seriesId
              ? {
                  ...
                  sharedElements: {
                    ...s.sharedElements,
                    characters: s.sharedElements.characters.filter(
                      (sc) => sc.characterId !== characterId,
                    ),
                  },
                  updatedAt: new Date(),
                }
                  ...
                  sharedElements: {
                    ...
                    characters: s.sharedElements.characters.map((sc) =>
                      sc.characterId === characterId
                        ? {
                            ...sc,
                            appearances: [...sc.appearances, appearance],
                          }
                        : sc,
                    ),
                  },
                    ...
                    characters: s.sharedElements.characters.map((sc) =>
                      sc.characterId === characterId
                        ? {
                            ...sc,
                            developmentArc: [
                              ...sc.developmentArc.filter(
                                (d) => d.bookNumber !== bookNumber,
                              ),
                              development,
                  ...
                  sharedElements: {
                    ...
                    characters: s.sharedElements.characters.map((sc) =>
                      sc.characterId === characterId
                        ? {
                            ...sc,
                            relationships: [...sc.relationships, relationship],
                          }
                        : sc,
                    ),
                  },
              ...
              ? {
                  ...
                  sharedElements: {
                    ...
                    worldBuilding: {
                      ...s.sharedElements.worldBuilding,
                      worldRules: [
                        ...s.sharedElements.worldBuilding.worldRules,
                        { ...rule, id: generateId() },
                      ],
                    },
                  },
            ...
            s.id === seriesId
              ? {
                  ...
                  sharedElements: {
                    ...
                    worldBuilding: {
                      ...s.sharedElements.worldBuilding,
                      worldRules: s.sharedElements.worldBuilding.worldRules.map(
                        (r) => (r.id === ruleId ? { ...r, ...updates } : r),
                      ),
                    },
                  },
                  updatedAt: new Date(),
              ...
              ? {
                  ...
                  sharedElements: {
                    ...
                    worldBuilding: {
                      ...s.sharedElements.worldBuilding,
                      worldRules:
                        s.sharedElements.worldBuilding.worldRules.filter(
                          (r) => r.id !== ruleId,
                        ),
                    },
                  },
            ...
            s.id === seriesId
              ? {
                  ...
                  sharedElements: {
                    ...s.sharedElements,
                    magicSystems: [
                      ...s.sharedElements.magicSystems,
                      { ...system, id: generateId() },
                    ],
                  },
                  updatedAt: new Date(),
                }
          ...
          series: state.series.map((s) =>
            s.id === seriesId
              ? {
                  ...
                  sharedElements: {
                    ...s.sharedElements,
                    magicSystems: s.sharedElements.magicSystems.filter(
                      (ms) => ms.id !== systemId,
                    ),
                  },
                  updatedAt: new Date(),
          ...
          series: state.series.map((s) =>
            s.id === seriesId
              ? {
                  ...
                  sharedElements: {
                    ...s.sharedElements,
                    locations: s.sharedElements.locations.filter(
                      (l) => l.id !== locationId,
                    ),
                  },
                  updatedAt: new Date(),
          ...
          series: state.series.map((s) =>
            s.id === seriesId
              ? {
                  ...
                  sharedElements: {
                    ...s.sharedElements,
                    factions: s.sharedElements.factions.filter(
                      (f) => f.id !== factionId,
                    ),
                  },
                  updatedAt: new Date(),
              ...
              ? {
                  ...
                  sharedElements: {
                    ...
                    worldBuilding: {
                      ...s.sharedElements.worldBuilding,
                      timeline: [
                        ...s.sharedElements.worldBuilding.timeline,
                        { ...event, id: generateId() },
                      ].sort(
                        (a, b) =>
            ...
            s.id === seriesId
              ? {
                  ...
                  sharedElements: {
                    ...
                    worldBuilding: {
                      ...s.sharedElements.worldBuilding,
                      timeline: s.sharedElements.worldBuilding.timeline
                        .map((e) =>
            ...
            s.id === seriesId
              ? {
                  ...
                  sharedElements: {
                    ...
                    worldBuilding: {
                      ...s.sharedElements.worldBuilding,
                      timeline: s.sharedElements.worldBuilding.timeline.filter(
                        (e) => e.id !== eventId,
                      ),
                    },
                  },
          ...
          series: state.series.map((s) =>
            s.id === seriesId
              ? {
                  ...
                  sharedElements: {
                    ...s.sharedElements,
                    terminology: s.sharedElements.terminology.filter(
                      (t) => t.id !== termId,
                    ),
                  },
                  updatedAt: new Date(),
          ...
          series: state.series.map((s) => ({
            ...
            books: s.books.map((b) =>
              b.id === bookId
                ? {
                    ...b,
                    plotThreads: b.plotThreads.filter(
                      (pt) => pt.id !== threadId,
                    ),
                  }
                : b,
          ...
          series: state.series.map((s) => ({
            ...
            books: s.books.map((b) =>
              b.id === bookId
                ? {
                    ...b,
                    characterArcs: b.characterArcs.filter(
                      (ca) => ca.id !== arcId,
                    ),
                  }
                : b,
      ...
      generateSeriesAnalytics: (seriesId) => {
        ...
        const analytics: SeriesAnalytics = {
          seriesId,
          totalWordCount: series.books.reduce(
            (sum, book) => sum + book.currentWordCount,
            0,
          ),
          averageBookLength:
          ...
          completionRate:
            series.books.length > 0
              ? series.books.filter((book) => book.status === "published")
                  .length / series.books.length
              : 0,
          characterCount: series.sharedElements.characters.length,
          locationCount: series.sharedElements.locations.length,
          plotThreadCount: series.books.reduce(
            (sum, book) => sum + book.plotThreads.length,
            0,
          ),
          ...
          characterDevelopment: series.sharedElements.characters.map(
            (char) => ({
          ...
          worldBuildingDepth: {
            ...
            consistency: 0.8,
            complexity:
              (series.sharedElements.magicSystems.length +
                series.sharedElements.factions.length) *
              0.1,
            originality: 0.7,
            integration: 0.8,
          },
      ...
      checkConsistency: (seriesId) => {
        ...
        series.sharedElements.characters.forEach((character) => {
          const developments = character.developmentArc.sort(
            (a, b) => a.bookNumber - b.bookNumber,
          );
          for (let i = 1; i < developments.length; i++) {
            if (
              developments[i].startingLevel < developments[i - 1].endingLevel
            ) {
              issues.push({
                id: generateId(),
                type: "warning",
        ...
        series.sharedElements.worldBuilding.worldRules.forEach((rule) => {
          const contradictoryRefs = rule.references.filter(
            (ref) => ref.importance === "breaks",
          );
      ...
      validateCharacterProgression: (seriesId, characterId) => {
        const state = get();
        const series = state.series.find((s) => s.id === seriesId);
        const character = series?.sharedElements.characters.find(
        ...
        for (let i = 1; i < levelProgression.length; i++) {
          if (levelProgression[i].level < levelProgression[i - 1].level) {
            issues.push(
              `Level decreases from book ${levelProgression[i - 1].book} to ${levelProgression[i].book}`,
            );
            suggestions.push(
              "Ensure character levels only increase or stay the same across books",
            );
          }
        }
        ...
        for (let i = 1; i < levelProgression.length; i++) {
          const levelGain =
            levelProgression[i].level - levelProgression[i - 1].level;
          if (levelGain > 20) {
            issues.push(
              `Large level jump (+${levelGain}) between books ${levelProgression[i - 1].book} and ${levelProgression[i].book}`,
            );
            suggestions.push(
              "Consider more gradual level progression between books",
            );
          }
        }
