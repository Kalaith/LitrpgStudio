...
export default function FocusTimer({
  ...
  const getNextSessionType = useCallback(():
    ...
    if (currentSession.type === "work") {
      const workSessionsCompleted =
        completedSessions.filter((s) => s.type === "work").length + 1;
      return workSessionsCompleted % settings.sessionsUntilLongBreak === 0
        ? "longBreak"
        : "shortBreak";
    }
  ...
  const completeSession = useCallback(() => {
    ...
    if (
      ...
      const messages = {
        ...
        longBreak:
          "Long break finished! You're refreshed and ready to continue.",
      };
    ...
    if (settings.soundEnabled) {
      const audio = new Audio(
        "data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhBTuWzPPI",
      );
      audio.play().catch(() => {});
    }
        ...
        <div className="text-center text-white">
          ...
          <div className="text-xl capitalize mb-8">
            {currentSession.type === "work"
              ? "Focus Time"
              : currentSession.type.replace(/([A-Z])/g, " $1")}
          </div>
    ...
    >
      ...
      <div className="text-center mb-8">
        {currentSession ? (
          <div>
            <div
              className={`inline-block px-4 py-2 rounded-full text-sm font-medium mb-4 ${getSessionColor(currentSession.type)}`}
            >
              {getSessionIcon(currentSession.type)}{" "}
              {currentSession.type === "work"
                ? "Focus Time"
