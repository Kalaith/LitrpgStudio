...
export const GlobalSearch: React.FC<GlobalSearchProps> = ({
  ...
  const { searchEntities, searchHistory, getEntityStats } =
  ...
  const searchResults = useMemo(() => {
    if (!query.trim()) {
      ...
      const recentResults: SearchResultItem[] = searchHistory
        .slice(0, 5)
            ...
            {searchResults.length > 0 ? (
              ...
              <div className="py-8 text-center text-gray-500">
                ...
                <p className="text-sm mt-2">
                  Try searching for characters, locations, items, or events
                </p>
              </div>
            ) : (
              <div className="py-8 text-center text-gray-500">
                ...
                <p className="text-sm mt-2">
                  Type @ to create links, or use quick actions
                </p>
              </div>
            )}
          </div>
          ...
          <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 rounded-b-lg">
            <div className="flex items-center justify-between text-xs text-gray-500">
              <div className="flex items-center space-x-4">
                ...
                {getEntityStats && (
                  <span>
                    {Object.values(getEntityStats()).reduce(
                      (sum, count) => sum + count,
                      0,
                    )}{" "}
                    items indexed
