...
export default function AdvancedTextEditor({
  ...
  const previewHtml = useMemo(() => {
    ...
    return DOMPurify.sanitize(editorContent.replace(/\n/g, "<br>"));
  }, [mode, editorContent]);
  ...
  return (
    ...
    >
            ...
            <div className="flex rounded-md overflow-hidden border border-gray-300 dark:border-gray-600">
              {["edit", "split", "preview"].map((modeOption) => (
                <button
                  key={modeOption}
                  onClick={() =>
                    setViewMode(modeOption as "edit" | "preview" | "split")
                  }
                  className={`
      ...
      <div
        className="flex h-96"
        style={{ height: focusMode ? "calc(100vh - 60px)" : "400px" }}
      >
        {/* Editor Panel */}
        {(viewMode === "edit" || viewMode === "split") && (
          <div
            className={`${viewMode === "split" ? "w-1/2 border-r border-gray-300 dark:border-gray-600" : "w-full"}`}
          >
            <Editor
              height="100%"
