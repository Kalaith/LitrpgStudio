        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 sm:gap-4">
                ...
                <div className="relative">
                  ...
                  >
                    <span className="truncate">
                      {currentSeries ? currentSeries.name : series[0].name}
                    </span>
                  ...
                  {showSeriesDropdown && (
                    <>
                      ...
                      <div className="absolute left-0 top-full mt-1 z-20 min-w-[200px] rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 shadow-lg py-1 overflow-hidden">
                        {series.map((s) => {
                          const isActive = currentSeries
                            ? String(s.id) === String(currentSeries.id)
                            : String(s.id) === String(series[0].id);
                          return (
                            <button
                              key={s.id}
                              onClick={() => {
              ...
              {isSaved && (
                <span className="text-sm text-success-600 dark:text-success-400 hidden sm:inline">
                  Saved
                </span>
              )}
              {user?.is_guest && (
