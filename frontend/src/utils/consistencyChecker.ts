...
export class ConsistencyChecker {
    ...
    nameMap.forEach((chars, _name) => {
      if (chars.length > 1) {
        issues.push({
          ...
          suggestions: [
            "Consider giving characters unique names",
            "Use nicknames or titles to differentiate",
          ],
        });
      }
    });
    ...
    allCharacters.forEach((char) => {
      ...
      if (char.backstory) {
        ...
        const worldLocations =
          story.worldBuilding.locations?.map((l) => l.name.toLowerCase()) || [];
        ...
        locationRefs.forEach((ref) => {
          if (!worldLocations.includes(ref.toLowerCase())) {
            issues.push({
              ...
              relatedIds: [char.id],
              suggestions: [
                "Add referenced locations to world building",
  ...
  private static checkTimelineConsistency(story: Story): ConsistencyIssue[] {
    ...
    const sortedEvents = [...story.timeline].sort((a, b) =>
    ...
    for (let i = 1; i < sortedEvents.length; i++) {
      ...
      if (
        prevEvent.importance === "critical" &&
        currentEvent.importance === "critical"
      ) {
        ...
        if (timeDiff < 7) {
          ...
          issues.push({
            ...
            relatedIds: [prevEvent.id, currentEvent.id],
            suggestions: [
              "Consider spacing major events",
              "Reduce importance of one event",
            ],
          });
        }
      }
    ...
    story.chapters.forEach((chapter) => {
      const chapterEvents = story.timeline.filter(
        (event) => event.chapter === chapter.id,
      );
      if (chapterEvents.length === 0 && chapter.content.length > 1000) {
        // Long chapter with no events
        issues.push({
          type: "info",
      ...
      locations.forEach((location) => {
        if (location.parentLocationId) {
          const parent = locations.find(
            (l) => l.id === location.parentLocationId,
          );
          if (!parent) {
            issues.push({
              type: "error",
              ...
              suggestions: [
                "Fix parent location reference",
                "Create missing parent location",
              ],
            });
            ...
            if (locationSizeIndex >= parentSizeIndex) {
              issues.push({
                ...
                suggestions: [
                  "Adjust location sizes",
                  "Review location hierarchy",
                ],
              });
            }
          }
  ...
  private static checkPlotConsistency(story: Story): ConsistencyIssue[] {
    ...
    const allChapterContent = chapters
      .map((c) => c.content)
      .join(" ")
      .toLowerCase();
    ...
    if (plotThreads > resolutions * 2) {
      issues.push({
        ...
        category: "plot",
        suggestions: [
          "Review unresolved mysteries",
          "Consider resolving some plot threads",
        ],
      });
    }
    ...
    if (
      ...
      story.wordCount > story.targetWordCount * 1.5
    ) {
      issues.push({
        type: "warning",
  ...
  private static generateSuggestions(
    ...
    if (errorCount > 0) {
      suggestions.push(
        "Address critical errors first to maintain story integrity",
      );
    }
