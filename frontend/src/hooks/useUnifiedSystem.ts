...
export const useUnifiedSystem = () => {
  const { getEntity, searchEntities, bulkImport } = useEntityRegistryStore();

  const { addEvent, getEventsByEntity, createView, setActiveView } =
  ...
  const initializeUnifiedSystem = () => {
    ...
    const { entities, relationships } = buildRegistryBatch(
      stories,
      characters,
      series,
    );
