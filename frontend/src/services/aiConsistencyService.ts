...
export class AIConsistencyService {
    ...
    const defaultRules: WorldRule[] = [
      {
        ...
        category: "physics",
        description:
          "Dead characters cannot perform actions unless explicitly resurrected",
        scope: "global",
      ...
      },
      {
        ...
        category: "magic",
        description:
          "Character power levels should follow consistent progression rules",
        scope: "global",
      ...
      },
      {
        ...
        category: "logic",
        description:
          "Characters should not know information they haven't learned",
        scope: "global",
  ...
  private async analyzeCharacterConsistency(
    ...
    for (const character of characters) {
      ...
      const deathEvents = timeline.filter(
        (e) =>
          (e.involvedEntities.some((entity) => entity.id === character.id) &&
            e.description.toLowerCase().includes("die")) ||
          e.description.toLowerCase().includes("death"),
      );

      const postDeathEvents = timeline.filter((e) => {
        const eventTime = e.timestamp.storyDay || 0;
        const latestDeath = deathEvents.reduce(
          (latest, death) => Math.max(latest, death.timestamp.storyDay || 0),
          0,
        );

        return (
          eventTime > latestDeath &&
          e.involvedEntities.some((entity) => entity.id === character.id) &&
      ...
      const locationEvents = timeline
        ...
        .sort(
          (a, b) => (a.timestamp.storyDay || 0) - (b.timestamp.storyDay || 0),
        );
      ...
      for (let i = 1; i < locationEvents.length; i++) {
        ...
        const timeDiff =
          (currEvent.timestamp.storyDay || 0) -
          (prevEvent.timestamp.storyDay || 0);
  ...
  private async analyzeTimelineConsistency(
    context: AIAnalysisContext,
  ): Promise<AIConsistencyIssue[]> {
    const issues: AIConsistencyIssue[] = [];
    ...
    for (let i = 0; i < sortedEvents.length - 1; i++) {
      ...
      if (currentEvent.dependencies && currentEvent.dependencies.length > 0) {
        for (const dep of currentEvent.dependencies) {
          ...
          if (
            dependentEvent &&
            (dependentEvent.timestamp.storyDay || 0) >
              (currentEvent.timestamp.storyDay || 0)
          ) {
            issues.push({
  ...
  private async analyzeWorldRuleViolations(
    context: AIAnalysisContext,
  ): Promise<AIConsistencyIssue[]> {
    const issues: AIConsistencyIssue[] = [];
    const { entities, timeline } = context.worldContext;
    ...
    for (const rule of this.worldRules.values()) {
      switch (rule.id) {
        case "power-progression": {
          ...
          const powerEvents = timeline.filter(
            (e) =>
          ...
          for (const event of powerEvents) {
            if (
              event.description.toLowerCase().includes("level") &&
              ...
              const associatedCharacters = event.involvedEntities.filter((e) =>
                entities.find(
                  (ent) => ent.id === e.id && ent.type === "character",
                ),
              );
              ...
              if (associatedCharacters.length > 0) {
                issues.push({
                  ...
                  description:
                    "Character gained power without visible training or explanation",
                  confidence: 0.6,
        ...
        case "economic-consistency": {
          ...
          const economicEvents = timeline.filter(
            (e) =>
          ...
          for (const event of economicEvents) {
            if (
              event.description.toLowerCase().includes("1000") &&
              ...
              issues.push({
                ...
                description:
                  "Character gained significant wealth without clear source",
                confidence: 0.4,
  ...
  private async analyzePowerProgression(
    context: AIAnalysisContext,
  ): Promise<AIConsistencyIssue[]> {
    const issues: AIConsistencyIssue[] = [];
    const { entities, timeline } = context.worldContext;
    ...
    for (const character of characters) {
      const characterEvents = timeline
        .filter((e) =>
          e.involvedEntities.some((entity) => entity.id === character.id),
        )
        .sort(
      ...
      const powerEvents = characterEvents.filter(
        (e) =>
          e.description.toLowerCase().includes("level") ||
      ...
      if (powerEvents.length >= 2) {
        ...
        for (let i = 1; i < powerEvents.length; i++) {
          ...
          if (
            prevEvent.description.toLowerCase().includes("level") &&
            ...
            issues.push({
              ...
              confidence: 0.3,
              entities: [character.id],
              evidence: [prevEvent.description, currEvent.description],
              suggestions: [
  ...
  private async analyzeKnowledgeConsistency(
    context: AIAnalysisContext,
  ): Promise<AIConsistencyIssue[]> {
    const issues: AIConsistencyIssue[] = [];
    const { entities, timeline } = context.worldContext;
    ...
    const knowledgeEvents = timeline.filter(
      (e) =>
    ...
    for (const event of knowledgeEvents) {
      ...
      if (characters.length > 0) {
        ...
        const suspiciousEvents = laterEvents.filter(
          (e) =>
  ...
  private async analyzeEmotionalConsistency(
    context: AIAnalysisContext,
  ): Promise<AIConsistencyIssue[]> {
    const issues: AIConsistencyIssue[] = [];
    const { timeline } = context.worldContext;
    ...
    const emotionalEvents = timeline.filter(
      (e) =>
    ...
    for (let i = 1; i < emotionalEvents.length; i++) {
      ...
      if (
        ...
        issues.push({
          ...
          description:
            "Character experienced quick emotional shift - consider adding transition",
          confidence: 0.3,
  ...
  private generateCacheKey(context: AIAnalysisContext): string {
    const entityIds = context.worldContext.entities
      .map((e) => e.id)
      .sort()
      .join(",");
    const timelineIds = context.worldContext.timeline
      .map((e) => e.id)
      .sort()
      .join(",");
