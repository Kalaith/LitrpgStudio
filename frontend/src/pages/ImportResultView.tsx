...
export default function ImportResultView({
  ...
  const payload = navigationState?.payload as
    | { importResult?: DraftImportResponse }
    | undefined;
  ...
  >("idle");
  const [checkResult, setCheckResult] = useState<ConsistencyCheckResult | null>(
    null,
  );
  ...
  const maxWords = Math.max(
    ...importResult.chapters.map((c) => c.word_count),
    1,
  );
  ...
  const runCheck = async () => {
    ...
    setCheckResult(null);
    try {
      const result = await apiClient.get<ConsistencyCheckResult>(
        `/series/${seriesId}/consistency-check`,
      );
  ...
  const nextSteps = [
    {
      Icon: UsersRound,
      title: "Characters",
      setupNote: "No cast list has been built for this book yet.",
      description:
    ...
    },
    {
      Icon: Clock3,
      title: "Timeline",
      setupNote: "No event timeline has been mapped for this book yet.",
      description:
    ...
    },
    {
      ...
      setupNote: "No setting or lore facts have been added for this book yet.",
      description:
      ...
      <div className="mx-auto max-w-4xl px-6 py-8 space-y-8">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
              <span className="font-medium text-gray-700 dark:text-gray-300">
                {importResult.book.title}
              </span>
              {importResult.series && <> &mdash; {importResult.series.title}</>}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            {
              label: "Chapters",
              value: String(importResult.summary.chapter_count),
            },
            {
              label: "Words",
              value: formatNumber(importResult.summary.word_count),
            },
          ].map(({ label, value }) => (
            <div
              key={label}
            ...
            <table className="min-w-full divide-y divide-gray-100 dark:divide-gray-700 text-sm">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-900/30">
                  <th className="px-4 py-2.5 text-left text-xs font-medium text-gray-500 dark:text-gray-400 w-8">
                    #
                  </th>
                  <th className="px-4 py-2.5 text-left text-xs font-medium text-gray-500 dark:text-gray-400">
                    Chapter
                  </th>
                  <th className="px-4 py-2.5 text-right text-xs font-medium text-gray-500 dark:text-gray-400 w-16">
                    Scenes
                  </th>
                  <th className="px-4 py-2.5 text-right text-xs font-medium text-gray-500 dark:text-gray-400 w-20">
                    Words
                  </th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {importResult.chapters.map((chapter) => (
                  ...
                  >
                    <td className="px-4 py-2 text-gray-400 dark:text-gray-500 tabular-nums">
                    ...
                    <td className="px-4 py-2 hidden sm:table-cell">
                      <div className="h-1.5 rounded-full bg-gray-100 dark:bg-gray-700">
                        <div
                          ...
                          style={{
                            width: `${Math.max(2, Math.round((chapter.word_count / maxWords) * 100))}%`,
                          }}
                        />
                      </div>
                    </td>
        ...
        <div>
          ...
          <p className="mt-1 mb-4 text-sm text-gray-500 dark:text-gray-400">
            Your draft is imported. Now set up the facts your story depends on
            so you can track continuity across chapters.
          </p>
          <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900/70 dark:bg-amber-950/30">
            <div className="flex items-start gap-3">
              <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg bg-amber-100 text-amber-700 dark:bg-amber-900/60 dark:text-amber-200">
                <AlertCircle className="h-5 w-5" aria-hidden="true" />
              </div>
              <div className="min-w-0">
                <h3 className="text-sm font-semibold text-amber-950 dark:text-amber-100">
                  Canon setup has not started for this book
                </h3>
                <p className="mt-1 text-xs leading-relaxed text-amber-800 dark:text-amber-200">
                  The chapter structure exists now, but Characters, Timeline,
                  and World Building still need review before continuity checks
                  can use this novel's facts.
                </p>
              </div>
            </div>
            <div className="mt-4 grid gap-2 sm:grid-cols-3">
              {nextSteps.map(({ title, setupNote }) => (
                <div
                  key={title}
                  className="rounded-lg border border-amber-200 bg-white/70 p-3 dark:border-amber-900/70 dark:bg-gray-900/40"
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="text-xs font-semibold text-gray-900 dark:text-white">
                      {title}
                    </span>
                    <span className="inline-flex items-center gap-1 rounded-full border border-amber-300 bg-amber-100 px-2 py-0.5 text-[11px] font-medium text-amber-800 dark:border-amber-700 dark:bg-amber-900/60 dark:text-amber-100">
            ...
            {nextSteps.map(({ Icon, title, description, action, view }) => (
              <button
                ...
                onClick={() =>
                  navigateToView(view, {
                    seriesId,
                    storyId: importResult.story.id,
                  })
                }
                className="group flex flex-col items-start gap-3 rounded-xl border border-amber-200 bg-white p-4 text-left transition-all hover:border-blue-400 hover:shadow-sm dark:border-amber-900/70 dark:bg-gray-800 dark:hover:border-blue-500"
              >
                <div className="flex w-full items-start justify-between gap-3">
                  ...
                  <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-0.5 text-[11px] font-medium text-amber-800 dark:bg-amber-900/50 dark:text-amber-100">
                    <CircleDashed className="h-3 w-3" aria-hidden="true" />
                    Needs setup
                  </span>
                </div>
                ...
                <p className="inline-flex items-center gap-1 text-xs font-medium text-blue-600 group-hover:underline dark:text-blue-400">
                  {action}
          ...
          <div className="flex flex-col sm:flex-row sm:items-start gap-4">
            <div className="flex-1">
              ...
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                Scan your imported chapters for potential continuity issues —
                contradictions, timeline gaps, and missing references.
              </p>
            </div>
            <button
              onClick={runCheck}
            ...
            <div className="mt-4 rounded-lg border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30 p-3">
              {checkResult.issues && checkResult.issues.length > 0 ? (
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 space-y-1">
                  <p>
                    {checkResult.message ?? "Check complete — no issues found."}
                  </p>
                  {checkResult.message
                    ?.toLowerCase()
                    .includes("coming soon") && (
                    <p className="text-gray-400 dark:text-gray-500">
                      Full continuity analysis is on the roadmap. For now, start
                      building out your characters and timeline above — that's
                      where the real continuity work happens.
                    </p>
                  )}
                </div>
              )}
