...
const availableWidgetTypes: {
...
}[] = [
  {
    type: "word_count",
    title: "Word Count",
    description: "Track your daily and total word count",
  },
  {
    type: "writing_streak",
    title: "Writing Streak",
    description: "Monitor your writing consistency",
  },
  {
    type: "character_overview",
    title: "Character Overview",
    description: "Quick view of your characters",
  },
  {
    type: "timeline_events",
    title: "Timeline Events",
    description: "Upcoming story events",
  },
  {
    type: "writing_goals",
    title: "Writing Goals",
    description: "Track your daily writing goals",
  },
  {
    type: "recent_activity",
    title: "Recent Activity",
    description: "Latest changes and edits",
  },
  {
    type: "story_progress",
    title: "Story Progress",
    description: "Overall story completion",
  },
  {
    type: "quick_stats",
    title: "Quick Stats",
    description: "Key project statistics",
  },
];
...
const DashboardView: React.FC = () => {
  ...
  const hasData =
    (seriesApi.series?.length ?? 0) > 0 ||
    (charactersApi.characters?.length ?? 0) > 0;

  // Grid layout calculations with responsive design
  const gridClasses = useMemo(() => {
    switch (gridSize) {
      case "compact":
        return "grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-8 gap-2";
      case "comfortable":
        return "grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 sm:gap-4";
      case "spacious":
        return "grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4 sm:gap-6";
      default:
        return "grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 sm:gap-4";
    }
  }, [gridSize]);
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between p-3 sm:p-4 lg:p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 gap-3 lg:gap-4">
        <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
          <div className="flex items-center space-x-2">
            <Grid
              size={20}
              className="text-primary-600 dark:text-primary-400 sm:hidden"
            />
            <Grid
              size={24}
              className="text-primary-600 dark:text-primary-400 hidden sm:block"
            />
        ...
        {!hasData ? (
          ...
          >
            ...
            <p className="text-gray-500 dark:text-gray-400 max-w-sm mb-8 text-base">
              Upload an existing draft and Writers Studio will organise your
              characters, timeline, and chapters automatically.
            </p>
            <button
          ...
          <div className="max-w-7xl mx-auto">
            <motion.div layout className={`grid ${gridClasses} auto-rows-min`}>
              <AnimatePresence>
                {widgets.map((widget) => (
                  <DashboardWidget
                    key={widget.id}
